...
## 12. פתרון תקלות ב-AWS Load Balancer Controller
*חלק זה מתעד את התקלות שחווינו בהגדרת ה-ALB Controller מול שירותי AWS.*

**1. שגיאת AWS Load Balancer Controller - MissingEndpoint**
* **בעיה:** לאחר הוספת ה-ALB Controller, לוג הפוד הראה שגיאת `MissingEndpoint: 'Endpoint' configuration is required for this service`. השגיאה נבעה מכך שהערך של `region` ו-`vpcId` לא עבר בצורה תקינה מה-`helmfile.yaml` מכיוון ששימוש במרכאות ובתחביר `{{ .Values.region }}` בבלוק `set:` גרם ל-Helmfile לא לפענח את הערך, וה-SDK של אמזון קיבל מחרוזת ריקה או שגויה.
* **פתרון:** במקום להשתמש בבלוק `set` שדורש התמודדות עם תבניות GoTemplate בעייתיות, השתמשנו בסקריפט `generate-values.sh` שלנו כדי לייצר קובץ ייעודי ומסודר בשם `alb.yaml` (המכיל את כל המשתנים כולל Region ו-VpcID). עדכנו את `helmfile.yaml` שיצביע על הקובץ החדש הזה ישירות באמצעות `values: - ./values/alb.yaml`.

**2. שגיאת AWS Load Balancer Controller - NoCredentialProviders**
* **בעיה:** לאחר שפתרנו את ה-Endpoint, הלוגים הראו שגיאת `NoCredentialProviders`. המשמעות היא שהפודים של ה-ALB רצו ללא הרשאות IAM תקינות מול אמזון, למרות שייצרנו IRSA (IAM Roles for Service Accounts) בטראפורם. הסיבה הייתה שה-ServiceAccount של הפוד לא קיבל את ה-Annotation הרלוונטי `eks.amazonaws.com/role-arn` בגלל שגיאת מיזוג קבצי תצורה ב-Helm, וגם לאחר שהיא תוקנה, הפודים הישנים שרצו לא התעדכנו. ההזרקה של הרשאות ב-EKS (באמצעות Webhook) קורית *רק* בזמן יצירת הפוד.
* **פתרון:** הזרקנו באופן מפורש את הגדרת ה-ServiceAccount עם ה-Annotation לקובץ `alb.yaml`. לאחר מכן, היה צורך **למחוק ידנית את הפודים התקועים**:
<div dir="ltr">
...
* **תוצאה:** הפודים שעלו מחדש קראו את ה-ServiceAccount המעודכן, ה-EKS Pod Identity Webhook זיהה את ה-Annotation, והזריק לפודים את משתני הסביבה הדרושים (`AWS_ROLE_ARN` וכו'). ה-Controller התחבר ל-AWS וייצר בהצלחה את ה-Application Load Balancer ואת ה-Target Groups.

## 13. פתרון תקלות ב-External-DNS

**שגיאת External-DNS - no EC2 IMDS role found**
* **בעיה:** ה-External-DNS הציג שגיאות בלוגים: `failed to list hosted zones... no EC2 IMDS role found`. בדיוק כמו ב-ALB, זה מצביע על כך שהפוד ניסה להתחבר ל-Route 53 אבל לא קיבל את הרשאות ה-IAM. בבדיקה ראינו שה-Annotation של ה-Role היה ריק (`""`) בקובץ `external-dns.yaml` בגלל חוסר התאמה במבנה הקבצים בעת שימוש ב-`helmfile`.
* **פתרון:** 
  1. הוספנו פקודת `sed` לתוך הסקריפט `scripts/generate-values.sh` שמזריקה את ה-IAM Role ARN ישירות לתוך קובץ ה-`values/external-dns.yaml`.
  2. הרצנו שוב את הסקריפט ועשינו `helmfile sync`.
  3. ביצענו ריסטארט לפוד (`kubectl rollout restart`) כדי שיקבל את ההרשאות החדשות.
* **תוצאה:** הלוגים חזרו עם `All records are already up to date`, מה שהוכיח שהפוד התחבר בהצלחה ל-AWS.


## 14. פתרון תקלות ב-Horizontal Pod Autoscaler (HPA)
*חלק זה מתעד את התקלות שחווינו כשניסינו להריץ בדיקות עומס (Load Testing) וה-HPA סירב לבצע Scale Up.*

**1. ה-HPA מציג `cpu: <unknown>/70%` ולא מבצע Scale**
* **בעיה:** בבדיקת מצב ה-HPA (`kubectl get hpa`) ראינו שהיעד למעבד (CPU) והזיכרון מופיעים כ-`<unknown>`, מה שאומר שה-HPA "עיוור" למשאבים של הפודים. פקודות כמו `kubectl top pods` גם הן החזירו שגיאות.
* **סיבת התקלה:** שירות ה-`metrics-server`, שאחראי לאסוף נתוני CPU/Memory מהשרתים, אינו מותקן כברירת מחדל באשכולות EKS של AWS. ללא שירות זה, שום כלי אוטומציה שמבוסס על מדדים לא יכול לפעול.
* **התיקון:** 
  הוספנו את ה-Chart הרשמי של ה-`metrics-server` לתוך קובץ ה-`helmfile.yaml` שלנו והתקנו אותו בקלאסטר (ב-namespace של `kube-system`).

**2. ה-HPA לא מצליח למצוא את ה-Deployment (שגיאת `FailedGetScale`)**
* **בעיה:** בלוגים ובהודעות ה-Events של ה-HPA הופיעה השגיאה:
<br/>
  `the HPA controller was unable to get the target's current scale: deployments/scale.apps "backend" not found`
<br/>
  במקביל, ה-HPA הציג שמספר ה-Replicas שלו עומד על `0` למרות שהיו פודים שרצו בפועל.
* **סיבת התקלה:** בקבצי ה-`hpa.yaml` ו-`vpa.yaml` של אפליקציות ה-frontend וה-backend, יעד הסקיילינג (scaleTargetRef) הוגדר בטעות לשם האפליקציה בלבד (למשל `backend`), בעוד שה-Deployment האמיתי שלנו נוצר עם הסיומת `deployment-` (למשל `backend-deployment`). ה-HPA חיפש Deployment שלא היה קיים.
* **התיקון:**
  עדכנו את קבצי התבניות (Templates) ב-Helm Charts:
<div dir="ltr">
...
```
  ...
  *(התיקון בוצע בצורה זהה גם עבור ה-VPA וגם ב-frontend).*
  לאחר הרצת `helm upgrade`, ה-HPA זיהה מיד את ה-Deployments, קרא את נתוני ה-`metrics-server`, והתחיל להוסיף רפליקות בצורה תקינה.
